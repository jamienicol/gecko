  ...
  switch (sdType) {
    ...
    case layers::SurfaceDescriptor::TSurfaceDescriptorAndroidHardwareBuffer: {
      ...
      ipc::FileDescriptor handle =
          std::move(const_cast<ipc::FileDescriptor&>(sd.handle()));
      RefPtr<layers::AndroidHardwareBuffer> buffer =
          layers::AndroidHardwareBuffer::FromFileDescriptor(
              std::move(handle), sd.bufferId(), sd.size(), sd.format());
...
bool GLBlitHelper::BlitImageToFramebuffer(layers::Image* const srcImage,
  ...
  switch (srcImage->GetFormat()) {
    ...
    case ImageFormat::ANDROID_IMAGE: {
      ...
      MOZ_ASSERT(image);
      auto buffer = image->GetHardwareBuffer();
      return Blit(buffer, destSize, destOrigin);
#else
      MOZ_ASSERT(false);
      return false;
