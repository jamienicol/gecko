...
void NativeLayerRootAndroid::SetLayers(
  ...
  if (newSublayers != mSublayers) {
    for (const RefPtr<NativeLayerAndroid>& layer : newSublayers) {
      if (!mSublayers.Contains(layer)) {
        mAddedSublayers.AppendElement(layer);
      }
    }
...
bool NativeLayerRootAndroid::CommitToScreen() {
  ...
  if (mMutatedLayers) {
    for (auto& layer : mAddedSublayers) {
      ASurfaceTransaction_reparent(transaction, layer->mSurfaceControl.get(),
                                   mSurfaceControl.get());
    }
    ...
    if (releasedBuffer != releasedBuffers.end()) {
      releasedBuffer->second.mSurface.match(
          ...
          [=](RefPtr<AndroidHardwareBuffer>& buffer) {
            // FIXME: plumb release fence back to client side (via
            // AsyncImagePipelineManager?)
            // FIXME: ensure AsyncImagePipelineManager is keeping us alive long
            // enough
            if (releaseFence != -1) {
              ::close(releaseFence);
            }
    ...
    it.second.mSurface.match(
        ...
        [&](const RefPtr<AndroidHardwareBuffer>& buffer) {
          printf_stderr(
              "jamiedbg Unhandled released buffer: nativeLayer: %p, sc: %p, "
              "buffer: %p\n",
              it.second.mLayer.get(), it.first, buffer.get());
        });
  }
...
std::ostream& operator<<(std::ostream& aStream,
  ...
  aBufferSource.match(
      ...
      [&](const RefPtr<AndroidHardwareBuffer>& buffer) {
        aStream << "AndroidHardwareBuffer [" << buffer.get() << "]";
      });
  return aStream;
}
...
NativeLayerAndroid::NativeLayerAndroid(
    ...
    : mMutex("NativeLayerAndroid"),
      mSurfaceControl(std::move(aSurfaceControl)),
      mIsOpaque(aIsOpaque),
      mFrontBuffer(RefPtr<AndroidHardwareBuffer>(nullptr)),
      mPrevFrontBuffer(RefPtr<AndroidHardwareBuffer>(nullptr)) {}
...
void NativeLayerAndroid::Update(
  ...
  // the surface zOrder?
  ASurfaceTransaction_setZOrder(aTransaction, mSurfaceControl.get(), aZOrder);

  nsTArray<ARect> damageRects;
  if (!mDirtyRegion.IsEmpty()) {
    for (auto iter = mDirtyRegion.RectIter(); !iter.Done(); iter.Next()) {
      const gfx::IntRect& r = iter.Get();
      damageRects.AppendElement(ARect{
          .left = r.x,
          .top = r.y,
          .right = r.XMost(),
          .bottom = r.YMost(),
    ...
    ASurfaceTransaction_setDamageRegion(aTransaction, mSurfaceControl.get(),
                                        damageRects.Elements(),
                                        damageRects.Length());
  ...
  if (mFrontBufferUpdated) {
    ...
    mFrontBuffer.match(
        [&](UniquePtr<HardwareBufferSurface>& surface) {
          ...
          ASurfaceTransaction_setBuffer(aTransaction, mSurfaceControl.get(),
                                        hardwareBuffer, fence);
        ...
        [&](RefPtr<AndroidHardwareBuffer>& buffer) {
          ...
          auto fenceFd = buffer->GetAndResetAcquireFence();

          ASurfaceTransaction_setBuffer(aTransaction, mSurfaceControl.get(),
                                        hardwareBuffer, fenceFd.release());
        });
  ...
  ASurfaceTransaction_reparent(aTransaction, mSurfaceControl.get(), aParent);
  ...
  if (surfaceRectClipped.IsEmpty() || bufferClip.IsEmpty()) {
    ...
    ASurfaceTransaction_setVisibility(aTransaction, mSurfaceControl.get(),
    ...
    ASurfaceTransaction_setGeometry(aTransaction, mSurfaceControl.get(), src,
                                    dest, ANATIVEWINDOW_TRANSFORM_IDENTITY);
  }
}
...
void NativeLayerAndroid::Remove(
  ...
  ASurfaceTransaction_reparent(aTransaction, mSurfaceControl.get(), nullptr);
}
...
void NativeLayerAndroid::AttachExternalImage(
  ...
  MutexAutoLock lock(mMutex);
  if (wr::RenderAndroidHardwareBufferTextureHost* host =
          aExternalImage->AsRenderAndroidHardwareBufferTextureHost()) {
    NativeLayerAndroidBufferSource newFrontBuffer =
        AsVariant(host->GetAndroidHardwareBuffer());
