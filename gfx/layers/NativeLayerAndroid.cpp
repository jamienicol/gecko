...
bool NativeLayerRootAndroid::CommitToScreen() {
  ...
  if (mMutatedLayers) {
    for (auto& layer : mAddedSublayers) {
      api->ASurfaceTransaction_reparent(
          transaction, layer->mSurfaceControl.get(), mSurfaceControl.get());
    }
...
void NativeLayerAndroid::Update(
  ...
  if (!mDirtyRegion.IsEmpty()) {
    ...
    api->ASurfaceTransaction_setDamageRegion(
        aTransaction, mSurfaceControl.get(), damageRects.Elements(),
        damageRects.Length());
  ...
  if (mFrontBufferUpdated) {
    ...
    mFrontBuffer.match(
        ...
        [&](RefPtr<AndroidHardwareBuffer>& buffer) {
          // FIXME: get acquire fence from AndroidHardwareBuffer
          AHardwareBuffer* const hardwareBuffer =
