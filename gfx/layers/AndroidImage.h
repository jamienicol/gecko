...
class AndroidImageReader
 ...
 public:
  ...
  // Creates an instance of this class attached to a GeckoImageReader Java
  // object. GeckoImageReader objects should be created using the
  // (Remote)SurfaceAllocator class.
  ...
  // Looks up an existing instance by its handle.
  ...
  // Returns the most recently acquired image, or null if no image has been
  // acquired.
  RefPtr<AndroidImage> GetCurrentImage();

 private:
  explicit AndroidImageReader(AImageReader* aImageReader);
  // FIXME: is the desctructor actually called?
  ~AndroidImageReader();
...
class AndroidImage {
 public:
  ...
  // Retrieves the image's hardware buffer. This remains owned by the image
  // unless a reference is explictly acquired.
