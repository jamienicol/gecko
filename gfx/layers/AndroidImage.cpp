...
RefPtr<AndroidImage> AndroidImageReader::AcquireLatestImage() {
  ...
  while (mPendingImages == 0) {
    if (lock.Wait(TimeDuration::FromMilliseconds(1000)) == CVStatus::Timeout) {
      gfxCriticalNote << "AndroidImageReader::AcquireLatestImage timed out waiting for image available callback";
  ...
  AImage* image;
  media_status_t res =
      api->AImageReader_acquireLatestImage(mImageReader, &image);
  if (res != AMEDIA_OK) {
    gfxCriticalNote << "AImageReader_acquireLatestImage failed:" << gfx::hexa(res);
    return nullptr;
  }
  mPendingImages = 0;
