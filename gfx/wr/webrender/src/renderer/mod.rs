...
use crate::gpu_types::{CopyInstance, PrimitiveInstanceData, SVGFEFilterInstance, ScalingInstance, SvgFilterInstance};
    ...
    fn get_texture_size(&self, texture: &TextureSource) -> DeviceIntSize {
        match *texture {
            ...
            TextureSource::External(TextureSourceExternal { index, .. }) => {
                // If UV coords are normalized then this value will be incorrect...
...
impl Renderer {
    ...
    fn handle_scaling(
        ...
        for (source, instances) in scalings {
            ...
            warn!("jamiedbg scaling: {:?}", instances);
