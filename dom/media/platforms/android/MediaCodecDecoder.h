...
class AsyncMediaCodec : public SupportsThreadSafeWeakPtr<AsyncMediaCodec> {
 public:
  ...
  bool IsAdaptivePlaybackSupported() const { return mIsAdaptivePlaybackSupported; }

 private:
  AsyncMediaCodec(AMediaCodec* aMediaCodec, Callbacks* aCallbacks,
                  bool aIsHardwareAccelerated);

  static nsTArray<java::sdk::MediaCodecInfo::LocalRef> FindMatchingCodecInfos(
      AMediaFormat* aFormat, bool aIsEncoder);
  void SetupAdaptivePlayback(java::sdk::MediaCodecInfo::Param aCodecInfo, AMediaFormat* aFormat);
  ...
  bool mIsAdaptivePlaybackSupported = false;
