...
RefPtr<AsyncMediaCodec> AsyncMediaCodec::Create(AMediaFormat* aFormat,
                                                ...
                                                bool aIsEncoder) {
  const auto codecNames = FindMatchingCodecNames(aFormat, aIsEncoder);
  for (const auto& name : codecNames) {
    ...
    return new AsyncMediaCodec(codec, aCallbacks, isHardwareAccelerated);
  }
...
AsyncMediaCodec::AsyncMediaCodec(AMediaCodec* aMediaCodec,
  ...
  if (__builtin_available(android 28, *)) {
    const media_status_t res =
        AMediaCodec_setAsyncNotifyCallback(mMediaCodec, callback, this);
...
/* static */
nsTArray<nsCString> AsyncMediaCodec::FindMatchingCodecNames(
    AMediaFormat* aFormat, bool aIsEncoder) {
  ...
  for (int i = 0; i < numCodecs; i++) {
    ...
    for (size_t type_idx = 0; type_idx < types->Length(); type_idx++) {
      ...
      nsCString name = info->GetName()->ToCString();
...
class MediaCodecVideoDecoder final : public MediaCodecDecoder {
 public:
  ...
  RefPtr<InitPromise> Init() override {
    ...
    mIsCodecSupportAdaptivePlayback = false;  // FIXME
